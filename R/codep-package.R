...
#' product, which is positive or negative when variables show similar or
#' opposing trends, respectively, are called codependence coefficients. These
#' eigenfunctions are obtained in three steps: 1) a distance matrice calculated
#' from the locations of samples in space (or the organisation of the sampling
#' schedule). 2) from that distance matrix, a matrix of spatial weights is
#' obtained; the same matrix as to calculate Moran's autocorrelation index,
...
#' 
#' The statistical significance of the codependence coefficients is tested using
#' parametric or permutational testing of a \eqn{\tau}{tau} statistic. The
#' \eqn{\tau}{tau} statistic is the product of the two Student's \eqn{t}{t}
#' statistics obtained from each of the two variables with a given
#' eigenfunction. The \eqn{\tau}{tau} statistic can take both positive and
#' negative values, thereby allowing one to perform one-directional or
#' two-directional testing. For multiple response variables, testing is
#' performed using the \eqn{phi}{phi} statistic instead. That statistics is the
#' distribution of the product of two Fisher-Snedocor F statistics (see
...
#' respectively. Methods are provided to print and plot \code{\link{cdp-class}}
#' objects (\code{\link{print.cdp}} and \code{\link{plot.cdp}}, respectively) as
#' well as summary (\code{\link{summary.cdp}}), fitted values
#' (\code{\link{fitted.cdp}}), residuals (\code{\link{residuals.cdp}}), and to
#' make predictions (\code{\link{predict.cdp}}).
#' 
...
#' calculate \code{\link{MCA}}. It returns a \code{\link{eigenmap-class}}
#' object. The package also features methods to print
...
#' 
#' The package also features an exemplary dataset \code{\link{salmon}}
#' containing  76 sampling site positions along a 1520 m river segment as well
#' as functions \code{\link{cthreshold}} and \code{\link{minpermute}}, which
#' calculates the testwise type I error rate threshold corresponding to a given
